...
| 2.4 | **Players tab (pilot) visual redesign**: maintainer review found the addon's whole visual system reads as an untheed scaffold, not a professional game-studio dashboard. Audited and confirmed: zero `box-shadow` anywhere (fully flat design); `.metric-card`/`.capability-card`/`.kpi-card` are visually identical boxes regardless of information type/importance; the declared `Inter` font is never actually loaded (silent fallback to system font); the one custom asset (`Dune Rise`) is used in exactly one place (hero title) and nowhere else; status colors (supported/unavailable/PvP/PvE) are plain text-color-only with no background tint or icon reinforcement, except one component (`res-combat-badge`) that does it properly, proving the pattern exists but isn't applied elsewhere; zero icons anywhere in the addon; only 2 CSS transitions exist in 613 lines (no hover states, no loading skeletons, no number-refresh animation). Scoped as a pilot on the Players tab first, to be rolled out to the remaining 9 tabs afterward. | addon | Players (pilot) | **In progress** — branch `feat/players-tab-visual-redesign`, commit `411d438`. Shipped so far: real elevation/shadow tokens, type scale, `Dune Rise` applied to all h2/h3 (was 1 place), differentiated metric/capability/kpi card treatments with hover states, real inline SVG icons on capability-status pills and combat badges (addon's first icon usage anywhere), zebra-striped/hoverable table rows. 52/52 tests pass (+2 new). Remaining: Players-tab-specific markup/layout pass (OPS Health Aggregate table, KPI Capability grid, Read-only KPI Panels sections themselves, beyond the shared CSS/JS already applied); user has not yet visually reviewed. |
