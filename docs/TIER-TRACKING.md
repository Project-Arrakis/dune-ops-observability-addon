...
| 2.3 | **Spice Melange UI corrections** (maintainer review of the Tier 0.18 rework): per-instance/sietch cards needed real combat-state accent coloring (not just the small badge); per-size table had a dashed-out "amount" column with no real backing data (should be dropped entirely, not dashed); instance-level spice total mislabeled "Remaining Spice" (overclaimed precision a live snapshot can't back up — renamed "Potential Spice" with an explanatory tooltip). | addon | Spice Melange | **Merged** — PR #72, `714da50`, released as v0.4.3. |
| 2.4 | **Players + Spice Melange visual redesign** (scope decision 2026-07-24: originally scoped as a "Players tab pilot," but the shared design-token/icon work in `web/addon.css`/`web/addon.js` genuinely spans both tabs — e.g. the new combat-badge icons live in Spice Melange's own `makeCombatBadge()`, not Players'. Maintainer decided to combine both tabs into one deliberate PR rather than artificially split shared-component work, and this becomes the pattern going forward for any future tab pair that shares a component). Original audit found the addon's whole visual system reads as an untheed scaffold, not a professional game-studio dashboard: zero `box-shadow` anywhere (fully flat design); `.metric-card`/`.capability-card`/`.kpi-card` visually identical regardless of information type; the declared `Inter` font never actually loaded; `Dune Rise` used in exactly one place; status colors (supported/unavailable/PvP/PvE) plain text-color-only except `res-combat-badge`; zero icons anywhere; only 2 CSS transitions in 613 lines. | addon | Players + Spice Melange | **Merged and released** — PR #73, `8de3575`, released as v0.4.4, live-installed and e2e-tested by maintainer. Shipped: addon-wide design tokens (elevation/shadow/type-scale), `Dune Rise` on all h2/h3, differentiated metric/capability/kpi cards with hover states, real inline SVG icons (capability-status pills + combat badges — addon's first icon usage anywhere), zebra-striped/hoverable tables, per-section instance-count badges (Spice Melange), elevated/hover-responsive instance cards with PvP/PvE glow-ring on hover. Found and deliberately deferred (not this PR): Players' OPS Health Aggregate table duplicates Overview's 4 metric-cards (data-architecture question, not visual); Combat tab's own copy-paste inline-style bug (Tier 2.5) left alone as its own separate fix. |
| 2.5 | **Visual-audit-adjacent bug fixes** (found during 2.4's audit, kept as separate small fixes per maintainer decision): (a) Combat tab's `index.html` eyebrow has a leftover inline `style="color:#c4b5fd"` (spice-purple) — copy-paste leftover from the Spice tab, factually wrong color for Combat; (b) the faction color-theming system's parent-inheritance is broken — `faction-tagger.js`'s `inheritFromParent()` sets `data-faction` on `<html>`, but the CSS only ever listens for `data-tagged-faction`, so inheriting the Console's active faction theme silently never applies any of the `:root` faction color variables. | addon | Combat, cross-cutting | **Not started** |
| 2.6 | **Meaningless/stale heading text across every tab** (maintainer question 2026-07-24, prompted by the Players tab's "Data source health / KPI Capability / Read-only support map" cluster): an independent audit of every `<section class="card">` across all 10 tabs found 4 of 15 heading clusters were clear internal-engineering filler, 4 more borderline, and 11 of 15 carried a stale `vX.X.0`-style eyebrow that doesn't correspond to this addon's real version (0.4.4) — leftover feature-milestone numbering, not real release tags (SOC's `v1.0.0` diverged furthest). Also found: Players' "OPS Health Aggregate" was the only card in the file with zero `.section-copy` fallback; Location's copy described a feature that will never exist (Location is permanently closed out-of-scope, not a temporary gap); Combat's copy didn't disclose the real, permanent PvP/PvE-always-shows-as-PvE limitation. | addon | All tabs | **Staged for PR** — PR #74 (`docs/rework-heading-clusters-all-tabs`), awaiting CI + maintainer review. Rewrote every eyebrow/h2/provider-label cluster addon-wide with plain-language text; removed all 11 stale version tags; provider-labels showing raw bridge-action names now lead with human framing (maintainer decision) while keeping the exact action name visible; added the missing Players section-copy; corrected Location's and Combat's copy to state their real, permanent limitations honestly. Also fixed a real false-zero bug found while touching the Inventory section: `Total Crafted` rendered a fabricated `0` (`?? 0`) for a field Core always returns as `null` by design — same F-1 anti-pattern already fixed elsewhere, not caught here since the whole-source-unavailable test case happened to also render a dash for an unrelated reason. |
...
| 3.3 | **Prometheus time-series graphs, in-addon** (maintainer idea, 2026-07-24, prompted by EA/Grafana blog post on dashboard design): `addonOpsPrometheusHealth()` currently does a single instant PromQL query (current CPU%, current memory, current target health) — Prometheus itself already retains 7 days of history (`METRICS_RETENTION_TIME:-7d` in `docker-compose.metrics.yml`) that is never queried. This addon would need its own range-query (`/api/v1/query_range`) support plus a charting approach (this addon has zero charting library today; only real options are inline SVG sparklines matching the zero-runtime-dependency pattern, or introducing the addon's first-ever runtime dependency). Scope is narrow: only meaningful for the Metrics(Prometheus) tab and possibly NOC Overview's currently-placeholder "Server Resources" panel — most other tabs (Players, Combat, Economy, Inventory, Spice Melange, SOC) are direct Postgres snapshot queries with no time-series storage at all, so graphing wouldn't apply to them. | core + addon | **Not started** | Deliberately sequenced after the Tier 2.4 visual redesign work completes. Distinct from 3.4 below — this is charts *inside the existing addon UI*, not a standalone dashboarding tool. |
| 3.4 | **Standalone Grafana deployment** (maintainer request, 2026-07-24): a real, full Grafana instance added to the stack (not addon-embedded sparklines — see 3.3) for genuine multi-panel, multi-metric dashboarding, using the Prometheus/node-exporter/cAdvisor/postgres-exporter stack that already exists (confirmed no Grafana anywhere in the current compose files or docs; explicitly listed as out-of-scope in `docs/E2E-METRICS-TESTING.md`). Maintainer explicitly requested this be designed via genuinely independent multi-agent invocations per-principal-role (not one agent role-playing multiple perspectives sequentially — the same anti-pattern named earlier this session for the post-Spice-Melange review), producing: (1) a design/architecture roadmap and (2) a Sonnet-5 implementation prompt, matching the existing `docs/prompts/PHASE-4-GOVERNANCE-AUTOMATION.md` pattern. | core + addon | **Not started** | Explicitly sequenced for after the current workstream (Players tab visual redesign + remaining Tier 2 items) completes — maintainer confirmed do-not-start-yet. |
