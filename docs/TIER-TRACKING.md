...
| 3.3 | **Prometheus time-series graphs** (maintainer idea, 2026-07-24, prompted by EA/Grafana blog post on dashboard design): `addonOpsPrometheusHealth()` currently does a single instant PromQL query (current CPU%, current memory, current target health) — Prometheus itself already retains 7 days of history (`METRICS_RETENTION_TIME:-7d` in `docker-compose.metrics.yml`) that is never queried. Confirmed no Grafana exists anywhere in this stack (absent from all compose files, explicitly listed as out-of-scope in `docs/E2E-METRICS-TESTING.md`) — this addon would need its own range-query (`/api/v1/query_range`) support plus a charting approach (this addon has zero charting library today; only real options are inline SVG sparklines matching the zero-runtime-dependency pattern, or introducing the addon's first-ever runtime dependency). Scope is narrow: only meaningful for the Metrics(Prometheus) tab and possibly NOC Overview's currently-placeholder "Server Resources" panel — most other tabs (Players, Combat, Economy, Inventory, Spice Melange, SOC) are direct Postgres snapshot queries with no time-series storage at all, so graphing wouldn't apply to them. | core + addon | **Not started** | Deliberately sequenced after the Tier 2.4 visual redesign work completes. |
