...
### Fixed
- **`pipeline/pre-push-gates`, `pipeline/pre-pr-check.sh`, and `pipeline/pr-ready-check.sh` fail open when trivy/shellcheck/gitleaks/ggshield aren't installed, despite each script's own header claiming "fails closed."** `pre-push-gates`'s trivy check swallowed a missing-binary error via `|| true` and fell through to an unconditional `pass "trivy clean"`; `pre-pr-check.sh` printed a neutral `SKIP:` line for shellcheck/gitleaks/trivy without incrementing its `FAILS` counter, so a run missing all three could still print "ALL CHECKS PASSED"; `pr-ready-check.sh` reported a missing `ggshield` via the `pass()` helper itself — a green checkmark for a scan that never ran. None of these are CI-enforced (they're local pre-push/pre-PR developer tooling), so the blast radius was a false all-clear on a contributor's machine, not a false-green badge on GitHub — but it directly contradicted the scripts' own documented fail-closed guarantee. All three now call `fail()` (or add an explicit `command -v` guard before running) when a required scanner binary is missing.

### Changed
- **Consolidated 5 separate workflow files independently running gitleaks/semgrep/trivy down to using one org-shared reusable workflow.** `ci.yml`'s "Security Scanning" job and `security-gates.yml`'s `semgrep-sast`/`secret-scan`/`trivy-filesystem` jobs were exact duplicates of `sast.yml`/`secret-scan.yml`/`filesystem-scan.yml` (same tools, same config) — removed, since none were themselves required checks. `sast.yml`/`secret-scan.yml`/`filesystem-scan.yml` now each call `Project-Arrakis/.github`'s new `reusable-security-scan.yml` instead of reimplementing the scan inline, so a fix there propagates here automatically instead of drifting. Branch protection's required check names updated to match the reusable-workflow call's naming (`gitleaks` → `gitleaks / gitleaks`, etc.). Also switched from `secrets: inherit` to explicitly passing only `SEMGREP_APP_TOKEN` — `secrets: inherit` was flagged by semgrep's own scan as a least-privilege violation.
- **Consolidated `.metric-card`/`.kpi-card`/`.health-card` into one `.stat-card` class with `--kpi`/`--health` modifiers (#146).** These were three separate class names for what is structurally one component — a bordered, elevated stat box — distinguished only by which of a bottom accent bar / secondary `<p>` note / smaller `<strong>` font-size each variant got. All 62 usages across `web/index.html` (54 base, 4 combo `metric-card health-card`, 4 `kpi-card`) migrated to `stat-card` (+ `stat-card--health` / `stat-card--kpi`), plus the 2 dynamically-created Spice Melange instance-card metric boxes in `web/addon.js`. **Zero visual change** — independently verified via a real jsdom computed-style comparison of all 62 `<article>` elements' border/padding/box-shadow/`<strong>` font-size+color/`<p>` color+font-size before and after the class rename: byte-identical on every field, for every element. New governance test (`test/governance.test.js`) asserts no stray reference to the old class names remains anywhere in `web/index.html`/`web/addon.js`/`web/addon.css`, to prevent silent re-introduction.
